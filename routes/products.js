...
// Only accept http(s) URLs — never store scripts or other schemes.
...
// { "Black": "https://...", "Camel": "https://..." } — drop anything that
// isn't a plain object, and clean/drop any entry whose URL doesn't pass
// cleanImageUrl. Keys with no valid URL are omitted rather than stored empty.
...
router.post("/", requireAdmin("full"), (req, res) => {
  ...
  db.prepare(
    `INSERT INTO products (id,name,description,price,stock,colors,sizes,gender,special_active,special_percent,image_url,color_images,created_by,created_at)
     VALUES (@id,@name,@description,@price,@stock,@colors,@sizes,@gender,@special_active,@special_percent,@image_url,@color_images,@created_by,@created_at)`
  ).run(row);
  res.status(201).json(serialize(row));
});

// Admin (full) only: update price, stock, sizes, colors, gender, images, or special/discount.
router.patch("/:id", requireAdmin("full"), (req, res) => {
  ...
  db.prepare(
    `UPDATE products SET name=@name, description=@description, price=@price, stock=@stock,
     ...
     image_url=@image_url, color_images=@color_images
     WHERE id=@id`
  ).run(next);
